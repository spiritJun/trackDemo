...
export default class InitMap {
    constructor(props) {
        ...
        this.el = props.el;
        this.MapUrlArray = [
            `https://webapi.amap.com/maps?v=1.4.15&callback=initAMap&key=${key}&plugin=AMap.TruckDriving,AMap.Scale,AMap.GraspRoad,AMap.LngLat,AMap.Driving,AMap.Geocoder`,
        ...
        return new Promise((resolve, reject) => {
            this.MapUILoader().then(AMap => {
                let map = new AMap.Map(this.el, {
                    autoRotation: true,
                    resizeEnable: true,
                    center: [116.397428, 39.90923],//设置地图中心点坐标 默认定位到皇宫
                    zoom: 14,
                });
                //地图绘制成功
                map.on('complete', () => {
